...
pub fn runWithPaths(allocator: std.mem.Allocator, io: std.Io, winter_path: []const u8, crop_path: []const u8, output_path: []const u8) !void {
    ...
    for (crops) |crop| for (townships) |township| {
...
pub fn addToFinalAccumulator(allocator: std.mem.Allocator, io: std.Io, input_root_path: []const u8, final_scores: *final_rating.Accumulator) !void {
    ...
    for (crops) |crop| {
        for (townships) |township| {
            try final_scores.addScore(
                strings.get(crop.crop_name_id),
                strings.get(township.township_id),
                .winter,
                @floatFromInt(winterColdToleranceScore(strings.get(crop.growth_habit_id), crop.critical_minimum_winter_temperature, township.minimum_temperature_quantile_05)),
            );
        }
    }
}
